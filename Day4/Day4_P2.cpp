...
int contarAdyacentes(vector<string> &matriz)
{
    int adyacentes = 0;
    int accesible = 0;
    ...
    for(int k = 0; k < (int)posiciones.size(); k++)
    {
        matriz[posiciones[k].first][posiciones[k].second] = '.';
    }
...
int main()
{
    ...
    do
    {
        eliminados = contarAdyacentes(matriz);
        solucion += eliminados;
    }
    while(eliminados != 0);
